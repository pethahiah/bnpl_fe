...
import DashboardSideBar, { INavItem } from "@/components/DashboardSideBar";
...
export default function DashboardComponentWrapper({ children, navItems }: { children: React.ReactNode, navItems: Array<INavItem> }) {
    ...
    <div className="w-[100vw] h-[100vh] flex flex-row justify-between items-start relative bg-[#F5F5F5]">
      ...
      <div className="min-h-[100vh] h-full flex-1 flex flex-grow flex-col">
        <div className="!sticky top-0 z-[5] lg:!relative w-[100%] h-[80px] bg-white px-[20px] py-[10px] flex justify-between items-center shadow-sm">
          ...
          <Link href={`${useGetBaseLink()}/settings/profile`} className="w-fit hidden lg:flex flex-row justify-between items-center">
          ...
          {/* mobile */}
          <Link href={`${useGetBaseLink()}/settings/profile`} className="flex lg:hidden justify-center items-center gap-[10px]">
            <ProfilemageCircle size={40} />
            <div className="flex flex-col gap-1 text-left h-auto">
