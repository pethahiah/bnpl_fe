...
function MultiStepFormWrapper(
  { children, steps, currentStep, setCurrentStep, customLaayoutComponent }:
    {
      children: ReactNode,
      steps: Array<Record<string, any>>,
      currentStep: number,
      setCurrentStep: Dispatch<SetStateAction<number>>,
      customLaayoutComponent?: () => JSX.Element
    }
) {
  const renderStepsCrumbs = () => steps.map((singleStep) => {
    const isNotCompleted = () => {
      const hasAnEmptyField = singleStep.inputs.find((i: any) => i.value === "" || i.value.length === 0)
  ...
  return (
    <div className='w-full flex min-h-[730px] relative flex-col-reverse md:flex-row'>
      ...
      <div className="flex-1 min-h-full p-10 gap-3 md:gap-1 bg-[#F55F641A]">
        {customLaayoutComponent && customLaayoutComponent()}
